  ...
  methods: {
    ...
    indexMethod(index) {
      return index + 1;
       ...
       this.ajax.commonApi2('get','X',this.api.statement.channelList+'?page='+ this.formInline.page +'&size='+ this.formInline.size)
        .then(res => {
          console.log('111',res);
        ...
        //  this.$router.push({path:'/merchant/merDetail',query:{page:this.formInline.page + 1,id:row.merchantId}})
         this.$router.push({path:'/merchant/merDetail',query:{page:this.formInline.page,id:row.merchantId}})
        //  console.log(row.id)
    },
    // 翻页
