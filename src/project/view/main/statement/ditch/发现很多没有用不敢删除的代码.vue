<template>
  <div class="c-content-inner">
        ...
        <el-form :model="template">
          <div class="editWidth">
            <el-form-item label="商户编码" :label-width="formLabelWidth" style="display: inline-block">
              <el-select v-model="form.coding" placeholder="商户编码">
    ...
    <el-dialog
      ...
      <el-form :model="rateConfigParam" :rules="rules">
      ...
      <el-tabs v-model="rateTab">
        ...
        <!-- 结算加费率设置 -->
        <el-tab-pane label="结算加费率设置" name="settlement">
          <el-row>
            <el-col :span="24">
              <div
                style="cursor:pointer;width: 100%;height: 30px;line-height: 30px; text-align: center;color: #fff;background: #409eff; border-radius: 5px;"
...
export default {
  ...
  methods: {
    ...
    sureModify() {
      if (!this.rateConfigParam.channelLeader) {
      ...
      if (booleans.includes(true)) {
        ...
        this.ajax
          .commonApi2(
            "post",
            "JSON",
            "/v3/channelCostRate/RateConfigSave",
            this.rateConfigParam
          )
          .then(res => {
            console.log(res);
    ...
    RateConfigPageList(row) {
      this.ajax
        .commonApi2(
          "get",
          "x",
          "/v3/channelCostRate/RateConfigPageList" +
            "?repChannelId=" +
        ...
        .then(res => {
          if (res.data.code == 0 && !!res.data.extra) {
            ...
            //this.rateConfigParam.costRatetings = res.extra.costRatetings
    ...
    handleSure() {
      ...
      this.$confirm(
        ...
        .then(() => {
          ...
          this.ajax
            .commonApi2("post", "JSON", this.api.statement.upCostRate, data)
            .then(res => {
              if (!res.data.code) {
                this.$message({
                  type: "success",
    ...
    useEdit(index, row) {
      this.ajax
        .commonApi2(
          "get",
          "X",
          this.api.statement.channelEnAble + "?id=" + row.id
        )
        .then(res => {
        ...
        .then(() => {
          this.ajax
            .commonApi2(
              "post",
              "X",
              this.api.statement.channelDelete,
              "?id=" + row.id
            )
            .then(res => {
              // console.log(res.data.extra)
    ...
    dataList() {
      this.ajax
        .commonApi2(
          "get",
          "X",
          this.api.statement.channelPageList +
            "?page=" +
        ...
        .then(() => {
          this.ajax
            .commonApi2(
              "get",
              "X",
              this.api.statement.channelDisAble + "?id=" + row.id
            )
            .then(res => {
              // console.log(res.data.extra)
              if (!res.data.code) {
