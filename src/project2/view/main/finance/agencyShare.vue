<template>
    <div class="c-content-inner">
         <el-form :inline="true" :model="formInline" class="demo-form-inline">
            <el-form-item label="选择报表日期" style="width:100%">
                <el-date-picker
            ...
            <el-form-item label="生成状态" placeholder="请选择生成状态">
                <el-select v-model="formInline.createState">
                    <el-option
            ...
            <el-form-item label="自动推送状态" placeholder="请选择自动推送状态">
                <el-select v-model="formInline.sendState">
                    <el-option
                        ...
                        <!-- slot-scope="scope" -->
                          ...
                          <el-table-column label="操作" width="320">
                            <template slot-scope="scope">
                                ...
                                <el-button
                                    :disabled="scope.row.preState !== 1 ? 'disabled' : false"
                                    size="mini" @click.native="createReqProfit(scope.row)">估算重新生成
                                </el-button>
                                <el-button
                                    :disabled="scope.row.preState !== 1 ? 'disabled' : false"
                                    size="mini" @click.native="sendReqProfit(scope.row)">系统重新推送
                                </el-button>
                            </template>
                        </el-table-column>
                    </el-table>
    ...
    export default {
        ...
        methods: {
            async repAgentProfitCount(){
                let res = await this.ajax.commonApi2("get","JSON",this.apiList.repAgentProfitCount)
            ...
            async getReqProfit(){
                ...
                let res = await this.ajax.commonApi2("post","JSON",this.apiList.getReqProfit,{
                    startDate,
                    endDate,
                    createState,
                    sendState,
            ...
            createReqProfit(item){
                ...
                this.$confirm('请确认是否重新生成代理商分润报表', '重新生成报表大约需要30分钟,你还要继续吗？', {
                    ...
                    let res = await this.ajax.commonApi2("post","JSON",this.apiList.createReqProfit,{
                        date:item.buildDate
                    })
